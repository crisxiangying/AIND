...
flags.DEFINE_string("output_eval_file", '/home/cxy/bert/data/wiki/expandNIL_putcasback/eval.txt', "Eval output file.")

flags.DEFINE_string("output_predict_file", '/home/cxy/bert/data/wiki/expandNIL_putcasback/5epchs_new_predict.txt', "Test output file.")
...
def create_zeshel_model(bert_config, is_training, input_ids, input_mask,
  ...
  with tf.variable_scope("loss"):
    ...
    # max_tensor = tf.reduce_max(output_layer[0, :, :], reduction_indices=0)
    # max_tensor = tf.reshape(max_tensor, [1, 768])
    # concat_output = tf.concat([output_layer[0, :, :], max_tensor], 0)
    # if FLAGS.train_batch_size>1:
    #   for i in range(1,FLAGS.train_batch_size):
    #     max_tensor = tf.reduce_max(output_layer[i,:,:], reduction_indices=0)
    #     max_tensor = tf.reshape(max_tensor, [1, 768])
    #     new_concat_output = tf.concat([output_layer[i,:,:], max_tensor], 0)
    #     concat_output = tf.concat([concat_output, new_concat_output], 0)
    # output_layer = concat_output
    # print('final output_layer shape: ' + str(output_layer.shape))
    # print('\n ----------------------------------------- \n')
    # ### new change
    ...
    logits = tf.reshape(logits, [-1, num_labels])  ########### 这里维度不需要+1了，不用留给NIL空位
    # 概率 用softmax计算
...
def main(_):
  ...
  if FLAGS.do_train:
      train_file = os.path.join(FLAGS.data_dir, "train_notnil.tfrecord")
  ...
  if FLAGS.do_eval:
    eval_file = os.path.join(FLAGS.data_dir, "train_notnil_smallsize.tfrecord")
    ...
    if FLAGS.use_tpu:
      eval_steps = 0
      # for fn in [eval_file]:
      for record in tf.python_io.tf_record_iterator(eval_file):
        eval_steps += 1
      print(eval_steps)
  ...
  if FLAGS.do_predict:
    predict_file = os.path.join(FLAGS.data_dir, "test_notnil.tfrecord")
    ...
    writer1 = tf.gfile.GFile(output_predict_file, "w")  ###新修改
    num_written_lines = 0
    tf.logging.info("***** Predict results *****")
    for (i, prediction) in enumerate(result):
        # print('iter for result:+++++++++++++++++++++++++++++++')
        probabilities = prediction["probabilities"]
        label_ids = prediction['label_ids']
        mention_guid = prediction['mention_guid']
        predict_index = np.argmax(probabilities)

        # if str(predict_index) != str(label_ids):  #现在对所有结果都输出，不是只输出预测错的!!!
        output_line1 = str(mention_guid) + '\n' \
                       + str(label_ids) + "\n" \
                       + str(predict_index) + "\n"  # \

        writer1.write(output_line1)  ##输出
        num_written_lines += 1
